...
test.describe(`@P1 @Functional @CartRecommendations Personalized Product Recommendations In Cart — ${config.displayName} on ${config.environment}`, () => {
  test('@P1 @Functional @CartRecommendations TC-004: Show recommendations quick link in cart navigation when recommendations exist', async ({
    page,
    loginModule,
    ...
    await test.step('Verify the recommendations quick link is visible in cart navigation', async () => {
